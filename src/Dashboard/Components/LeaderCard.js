      ...
      <div className='col-12 col-md-3 mb-4'>
        <div style={{ paddingBottom: '10px', borderRadius:'15px', background: 'rgba(255,255,255,0.5)', borderTop: 'none', marginBottom: '10px'}}>
            ...
            <div style={{ marginTop: '13px'}}>
              <TeamName rank={firstMedal} name={arr[0].name} score={ (arr[0].value / arr.length * 100).toFixed(2) } />
              <TeamName rank={secondMedal} name={arr[1].name} score={ (arr[1].value / arr.length * 100).toFixed(2) } />
              <TeamName rank={thirdMedal} name={arr[2].name} score={ (arr[2].value / arr.length * 100).toFixed(2) } />
            </div>
        </div>
      </div>
    )
